...
class MooseDataFrame(object):
    ...
    def update(self):
        ...
        if (file_exists and
            ...
            self.clear()
            # message.mooseDebug("The csv file {} exists but is old ({})
            # compared to the run start time ({}).".format(self.filename,
            # os.path.getmtime(self._filename), self._run_start_time),
            # debug=True)
            ...
            self.clear()
            # message.mooseDebug(
            #   "The csv file {} does not exist.".format(self._filename))
            ...
            if modified != self._modified:
                ...
                try:
                    ...
                    if self._add_peacock_index:
                        series = pandas.Series(self._data.index,
                                               index=self._data.index)
                        self._data.insert(0, 'index (Peacock)', series)
                    # message.mooseDebug("Reading csv file: {}".format(
                    #    self._filename))
                except Exception:
                    self.clear()
                    # message.mooseDebug("Unable to read file {} it likely does
                    # not contain data.".format(self._filename))
